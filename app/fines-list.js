...
import { selectFinesArray, removeFine, clearAllFines } from '../store/finesSlice';
...
export default function FinesListScreen() {
    ...
    Alert.alert(
      ...
      [
        ...
        },
        {
          text: t('clearAll'),
          onPress: () => {
            dispatch(clearAllFines());
            // Also clear from AsyncStorage
            AsyncStorage.removeItem('fines')
              .catch(error => console.error('Error removing fines from AsyncStorage:', error));
          },
          style: 'destructive'
        }
      ]
  ...
  const renderFineItem = ({ item }) => {
    const resultColor = item.analysis?.result === 'correct' 
      ? COLORS.success 
      : item.analysis?.result === 'partially' 
        ? COLORS.warning 
        : COLORS.error;
    
    const resultText = item.analysis?.result === 'correct' 
      ? t('inYourFavor') 
      : item.analysis?.result === 'partially' 
        ? t('partiallyFavor') 
        : t('notInFavor');
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerButtons}>
          ...
          <TouchableOpacity style={styles.addButton} onPress={() => router.push('/custom-case')}>
            <Text style={styles.addButtonText}>+ {t('reportNewFine')}</Text>
          </TouchableOpacity>
        </View>
      </View>
      ...
      {fines.length > 0 ? (
        ...
        <View style={styles.emptyState}>
          ...
          >
            <Text style={styles.reportButtonText}>{t('reportNewFine')}</Text>
          </TouchableOpacity>
        </View>
      )}
